 ...
 *
 * <p>
 *     Every {@link RobotPlace} having the valid position on X axis and Y axis with the direction can be treated as a
 *     defined state in DFA. So the actions that the Robot receives can be treated as the transition conditions among the
 *     states.
 *     The transition table is a two-dimension array. The row represents all the possible RobotPlace, which is the int value
 *     calculated by it position on X, Y axis and the direction. The column are the transition conditions, which are the
 *     actions in this case.
 *
 *     For example, the value of transition[0][1] is 36. It means that for the RobotPlace(0, 0, EAST), with action "LEFT",
 *     its new state is 36, which represents the (0, 0, NORTH).
 *
 *     Through the DFA, the lookup and determination for chain of the actions is very fast.
 * </p>
 *
 */
public class StateService {
  ...
  //ToDo: make these two variables as outer parameters that can change
  public static final int X_LENGTH = 5;
  public static final int Y_LEGNTH = 5;

  public static final int INVALID_STATE = -1;

  // The change table on X axis based on the action. The row is the direction, and the column is the action
  private static final int[][] X_MOVE_CHANGE = new int[Direction.values().length][Action.values().length];
  static {
  ...
  private static final int[][] Y_MOVE_CHANGE = new int[Direction.values().length][Action.values().length];
  static {
  ...
  private static final int[][] DIRECTION_CHANGE = new int[Direction.values().length][Action.values().length];
  static {
    DIRECTION_CHANGE[Direction.EAST.getValue()][Action.LEFT.getValue()] = Direction.NORTH.getValue();
    DIRECTION_CHANGE[Direction.NORTH.getValue()][Action.LEFT.getValue()] = Direction.WEST.getValue();
    DIRECTION_CHANGE[Direction.SOUTH.getValue()][Action.LEFT.getValue()] = Direction.EAST.getValue();
    DIRECTION_CHANGE[Direction.WEST.getValue()][Action.LEFT.getValue()] = Direction.SOUTH.getValue();

    DIRECTION_CHANGE[Direction.EAST.getValue()][Action.RIGHT.getValue()] = Direction.SOUTH.getValue();
    DIRECTION_CHANGE[Direction.NORTH.getValue()][Action.RIGHT.getValue()]  = Direction.EAST.getValue();
    DIRECTION_CHANGE[Direction.SOUTH.getValue()][Action.RIGHT.getValue()]  = Direction.WEST.getValue();
    DIRECTION_CHANGE[Direction.WEST.getValue()][Action.RIGHT.getValue()]  = Direction.NORTH.getValue();
    ...
    DIRECTION_CHANGE[Direction.NORTH.getValue()][Action.MOVE.getValue()]  = Direction.NORTH.getValue();
    DIRECTION_CHANGE[Direction.SOUTH.getValue()][Action.MOVE.getValue()]  = Direction.SOUTH.getValue();
    ...
    DIRECTION_CHANGE[Direction.EAST.getValue()][Action.REPORT.getValue()] = Direction.EAST.getValue();
    DIRECTION_CHANGE[Direction.NORTH.getValue()][Action.REPORT.getValue()]  = Direction.NORTH.getValue();
    DIRECTION_CHANGE[Direction.SOUTH.getValue()][Action.REPORT.getValue()]  = Direction.SOUTH.getValue();
    DIRECTION_CHANGE[Direction.WEST.getValue()][Action.REPORT.getValue()]  = Direction.WEST.getValue();
  }
  ...
  /**
   * Get the RobotPlace based on the given int state
   *
   ...
   *
   * <p>
   *     The row are the states calculated for very possible {@link RobotPlace}, and the column are the actions.
   * </p>
   *
   * @return the two-dimension array
   */
  public static int[][] buildTransitionTable() {
    int[][] transition = new int[X_LENGTH * Y_LEGNTH * Direction.values().length][Action.values().length];
